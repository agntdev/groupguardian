...
composer.command("ban", async (ctx) => {
  ...
  if (!replyTo?.from) {
    await ctx.reply("Reply to a user's message with /ban <reason> to ban them.", {
  ...
  try {
    ...
    await ctx.reply(
      `🚫 ${targetName} was banned by ${actorName}.\nReason: ${reason}`,
    ...
    await ctx.reply("Couldn't ban that user. Make sure I have the right admin permissions.");
  }
});
